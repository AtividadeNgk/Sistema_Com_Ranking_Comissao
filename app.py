...
@app.route('/api/analytics/bots', methods=['GET'])
def get_analytics_bots():
    if session.get("auth", False):
        bot_stats = manager.get_all_bots_faturamento()
        
        total_faturamento = sum(bot['total'] for bot in bot_stats)
        total_comissao = sum(bot['comissao'] for bot in bot_stats)
        total_vendas = sum(bot['vendas'] for bot in bot_stats)
...
@app.route('/api/analytics/comissoes', methods=['GET'])
def get_analytics_comissoes():
    if session.get("auth", False):
        ...
        comissoes = manager.get_comissoes_por_periodo(dias)
        
        total_periodo = sum(c[1] for c in comissoes) if comissoes else 0
