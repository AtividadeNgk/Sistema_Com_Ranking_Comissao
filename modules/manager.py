...
def get_all_bots_faturamento(start_date=None, end_date=None):
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    SELECT 
        ...
        SUM(CAST(json_extract(p.plano, '$.value') as REAL)) as total
    FROM PAYMENTS p
    JOIN BOTS b ON p.bot = b.id
    ...
    bot_stats = []
    for row in results:
        bot_details = check_bot_token(row[1])
        bot_stats.append({
            'bot_id': row[0],
            'bot_name': bot_details['result'].get('first_name', 'Bot') if bot_details else 'Bot',
            'bot_username': bot_details['result'].get('username', 'indefinido') if bot_details else 'indefinido',
            'vendas': row[2],
            'total': row[3] or 0,
            'comissao': (row[3] or 0) * 0.01,
            'ticket_medio': (row[3] or 0) / row[2] if row[2] > 0 else 0
        })
    
    return bot_stats

def get_comissoes_por_periodo(dias=7):
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    SELECT 
        DATE(p.id) as data,
        SUM(CAST(json_extract(p.plano, '$.value') as REAL)) * 0.01 as comissao_diaria
    FROM PAYMENTS p
    WHERE p.status = 'finished'
    AND p.id >= date('now', '-' || ? || ' days')
    GROUP BY DATE(p.id)
    ORDER BY data DESC
    """
